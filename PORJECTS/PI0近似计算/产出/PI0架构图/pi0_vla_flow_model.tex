...
\node[vlbox] (itokens) at (10.55,14.15) {Image\\Tokens};

\node[vlbox] (lang) at (1.45,10.35) {Language\\Instruction};
\node[vlbox] (embed) at (4.78,10.35) {Token\\Embedding};
\node[vlbox] (ttokens) at (7.85,10.35) {Text\\Tokens};

\node[greenbox] (robot) at (1.35,6.55) {Robot\\State};
...
\node[vlbox, minimum width=2.2cm, minimum height=6.95cm, text width=1.9cm] (concat) at (11.55,9.65)
  {Concatenate\\[2pt] Image Tokens\\[3pt] +\\[3pt] Text Tokens\\[3pt] +\\[3pt] State Token};
...
\draw[flow] (itokens.east) -| (concat.north);
...
\node[tinyblue] (rms1) at (16.15,13.85) {RMSNorm};
\node[tinyblue, minimum height=1.05cm] (mha) at (16.15,11.92) {Multi-Head\\Attention};
\node[tinyblue] (add1) at (16.15,9.86) {Add \& Residual};
\node[tinyblue] (rms2) at (16.15,8.23) {RMSNorm};
\node[tinyblue] (ffn) at (16.15,6.51) {Feed Forward};
\node[tinyblue] (add2) at (16.15,4.89) {Add \& Residual};
...
\draw[flow] ([xshift=0.25cm]rms1.east) -- ++(0.55,0) |- ([xshift=0.05cm]add1.east);
\draw[flow] ([xshift=0.25cm]rms2.east) -- ++(0.55,0) |- ([xshift=0.05cm]add2.east);
\draw[plainline] (15.0,14.72) -- (14.45,14.72) -- (14.45,3.88) -- (15.0,3.88);
\draw[decorate,decoration={brace,amplitude=4.5pt}] (13.83,3.55) -- (13.83,14.45)
  node[midway,left=6pt,font=\sffamily\bfseries\footnotesize] {Nx};
...
\node[greenbox, minimum width=2.65cm, text width=2.32cm] (noisy) at (24.02,14.00)
  {Noisy Action\\Chunk $d_t$};
...
\draw[flow] ([xshift=0.20cm]ar1.east) -- ++(0.55,0) |- ([xshift=0.04cm]aa1.east);
\draw[flow] ([xshift=0.20cm]ar2.east) -- ++(0.55,0) |- ([xshift=0.04cm]aa2.east);
\draw[decorate,decoration={brace,amplitude=4.5pt,mirror}] (23.75,5.45) -- (23.75,12.15)
  node[midway,left=6pt,font=\sffamily\bfseries\footnotesize] {Nx};

\draw[flow] (hidden.east) -- (expert.west);
\draw[flow] (noisy.south) -- ++(0,-0.50) -| ([xshift=-0.72cm]expert.north);

\node[greenbox, minimum width=3.0cm, text width=2.65cm] (timestep) at (22.50,3.60)
  {Flow Timestep $t$};
\draw[flow] (timestep.north) -- ++(0,0.92) -| ([xshift=-0.75cm]expert.south);

\node[callout, minimum width=2.45cm] (coGeluA) at (25.40,4.80) {GELU / SiLU};
...
\node[callout, minimum width=2.10cm, text width=1.85cm] (stepred) at (29.10,4.90)
  {Flow Step\\Reduction};
\draw[optarrow] (stepred.north) -- (iter.south);

\node[orangebox, minimum width=3.10cm, text width=2.78cm] (final) at (32.18,3.42)
  {Final Continuous\\Action Chunk};
\node[orangebox, minimum width=3.10cm, text width=2.78cm] (control) at (32.18,1.14)
  {Robot Control\\Execution};

\draw[flow] (iter.south) -- ++(0,-0.42) -| (final.north);
